...
DECLARE
  ...
  v_expirado BOOLEAN;
BEGIN
  -- Buscar token (alias `t` para evitar choque con la columna id_cita
  -- declarada en RETURNS TABLE → 42702 column reference is ambiguous).
  ...
  -- Validación exitosa - confirmar cita (también con alias `c`)
  UPDATE cita c
  SET confirmado = TRUE,
      estado     = 'pendiente'
